        ...
        setData: function (data)
        {
            ...
            if (g.autoEditors && g.autoEditors.length)
            {
                $(g.autoEditors).each(function ()
                {
                    var name = this.name;
                    var control = this.control;
                    if(control.type=="Radio"){//zx-form表单填充radio
                    	if(control.element.value==g.data[name]){//如果是当前值
...
(function ($)
{
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("        <div class='l-panel-topbar' style='display:none'><div class='l-panel-topbarinner'></div></div>");
        ...
        _calculateGridBodyHeight: function (h)
        {
            ...
            if (p.title) h -= 24;
            if (p.usePager && p.pagerRender || !p.scrollToPage) h -= g.toolbar.outerHeight()+3;//zx,分页，gridbody-3
...
(function ($)
{
    ...
    $.ligerui.controls.Radio.ligerExtend($.ligerui.controls.Input, {
        ...
        {
        	var g = this;
        	return $("input[name='"+g.element.name+"']:checked").val();//zx--返回Radio的选中值
