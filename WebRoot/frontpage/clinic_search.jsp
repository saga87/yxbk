...
.layui_nav .li_item{line-height: 45px}
  ...
  <% 
 String clinic_name = (String)request.getParameter("clinic_name");
	%>
...
var clinic_name = '<%=clinic_name%>';
  ...
  var pernum = 10;
  	...
  	$(function (){
		$("#clinictypename").text(clinic_name+"  的搜索结果");
  	$.ajax({
		...
		$.ajax({
               url: "front/web/clinic/searchClinics",
               data: {page:1,rows:pernum,clinic_name:clinic_name},
               dataType : "json",  
               type : "POST",
               success: function(ctt){        
               		var sum = ctt.length;
