...
.layui_nav .li_item{line-height: 80px}
  ...
  <% 
 String video_name = (String)request.getParameter("video_name");
	%>
...
var video_name = '<%=video_name%>';
  ...
  var pernum = 10;
  	...
  	$(function (){
		...
		$("#videotypename").text(video_name+"  的搜索结果");
  	...
  	$.ajax({
		...
		$.ajax({
               url: "front/web/video/searchVideos",
               data: {page:1,rows:pernum,video_name:video_name},
               dataType : "json",  
               type : "POST",
               success: function(ctt){        
               		var sum = ctt.length;
