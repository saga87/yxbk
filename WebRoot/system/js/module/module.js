	...
	$("#module_level").combobox({
		...
		onSelect:function(v){
			...
			if(1==v.value){
				...
				$("#module_parent_id").combotree({required: true,url:'wkrjsystem/wkrjModule/getGridInfo'});
			}else{
...
function saveModule(){
	...
	var m =top.$("iframe[src='wkrjsystem/wkrjModule/getPage']")[0].contentWindow;
...
function module_delRow(row){
    ...
    $.messager.confirm('信息提示','您确定要删除此记录吗?',function(r){
        if (r){
            $.post('wkrjsystem/wkrjModule/delModule',
...
function module_addModule(){
	...
	url='wkrjsystem/wkrjModule/addModule';
...
function module_editModule(){
    ...
    if (row){
        ...
        url='wkrjsystem/wkrjModule/updateModule';
    }else{
...
function module_addPerm(){
	...
	url='wkrjsystem/wkrjPermission/addPermission';
	$("#perm_icon").hide();
...
function module_editPerm(){
    ...
    if (rows.length>0){
        ...
        url='wkrjsystem/wkrjPermission/updatePermission';
    }else{
    	$.messager.alert('操作提示', '请选择您想修改的记录');
    }
...
function module_delPerm(){
    ...
    if (row.length>0){
    	...
    	$.messager.confirm('信息提示','您确定要删除此记录吗?',function(r){
        if (r){
	            $.post('wkrjsystem/wkrjPermission/delPermission',
